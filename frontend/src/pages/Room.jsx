...
export default function Room({ socket, roomData, onLeave }) {
  ...
  async function createPeerConnection(remoteSocketId, localStream) {
    ...
    if (initiator) {
      ...
      dc.onopen = () => console.log('datachannel open to', remoteSocketId);
  ...
  function leaveRoom() {
    ...
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:3000'}/api/leave`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ socketId: socket.id })
    }).catch(() => {});
    onLeave();
  }
  ...
  const handleJoinClick = () => {
    alert(`Joining room: ${roomData.roomId}`);
  };
          ...
          <div className="local">
            <video 
              ...
              style={{
                width: '100%',
                height: '100%',
                backgroundColor: localStream ? 'transparent' : '#f0f0f0',
